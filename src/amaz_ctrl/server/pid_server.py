...
class PID_server(AmazingServer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pid_parameters : dict, optional
            Dictionary containing 'p', 'i', and 'd' gains, by default {"p": 1, "i": 0, "d": 0}
    ...
    def set_pid_parameters(self, p:float=None, i:float=None, d:float=None):
        ...
        old_pid_value = (self.kp, self.ki, self.kd)
        if p is not None:
            self.kp = p
        if i is not None:
            self.ki = i
        ...
        try:
            ...
            self.kp, self.ki, self.kd = old_pid_value
            self.log.critical(
                f"Failed to set the PID values (p={p}, i={i}, d={d}). Old values {old_pid_value} are kept."
            )
    ...
    def compute_output(self):
        ...
        pid = self.kp * error + self.ki * self.int_error + self.kd * diff_error
        return pid
    ...
    def check_output(self, output):
        ...
        if output >  max(self_output_limits):
            return max(self_output_limits)
        if  output <  min(self_output_limits):
            return min(self_output_limits)
        return output
